...
## Requirements
- Given an input of a typical directory structure
- And a file that represents command line entries and their outputs
    ...
    - 123 abc means that the current directory contains a file named abc with size 123.
    - dir xyz means that the current directory contains a directory named xyz.
- The total size of a directory is the sum of the sizes of the files it contains, directly or indirectly (this process can count files more than once)
- Find all the directories with a total size of at most 100000
- Return the _sum_ of those directory sizes

## Pseudocode

```
while(input) {
    // switch on input line type commands
    // build up tree representation of directory structure
